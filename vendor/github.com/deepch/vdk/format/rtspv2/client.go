...
type RTSPClient struct {
	...
	seq int // auto increment in client.request

	// setted in client.request, search for res["Content-Base"]
	control string
	// setted in client.request, search for res["Session"]
	session string

	// setted in client.request, search for res["RTP-Info"]
	startVideoTS int64
	// setted in client.request, search for res["RTP-Info"]
	startAudioTS int64
	...
	mediaSDP []sdp.Media // client.request, method == DESCRIBE, res["Content-Length"]
	SDPRaw   []byte      // client.request, method == DESCRIBE, res["Content-Length"]

	conn   net.Conn
	connRW *bufio.ReadWriter // used to read & write to conn

	pURL *url.URL // setted in Dial -> parseSetURL

	headers map[string]string

	// searched for res["WWW-Authenticate"]
	clientDigest bool   // setted in client.request
	clientBasic  bool   // setted in client.request
	realm        string // setted in client.request
	nonce        string // setted in client.request
	username     string // setted in Dial -> parseSetURL
	password     string // setted in Dial -> parseSetURL
...
func Dial(options RTSPClientOptions) (*RTSPClient, error) {
	...
	err := client.parseSetURL(html.UnescapeString(client.options.URL))
	// client.pURL setted
	if err != nil {
		return nil, err
	}
	...
	for _, media := range client.mediaSDP {
		if (media.AVType != VIDEO && media.AVType != AUDIO) ||
			(client.options.DisableAudio && media.AVType == AUDIO) {
			continue
		}

		err = client.request(SETUP, map[string]string{"Transport": "RTP/AVP/TCP;unicast;interleaved=" + strconv.Itoa(ch) + "-" + strconv.Itoa(ch+1)}, client.ControlTrack(media.Control), false, false)
		if err != nil {
			return nil, err
		}
		...
		if media.AVType == VIDEO {
			if media.Type == av.H264 {
				if len(media.SpropParameterSets) > 1 {
					if codecData, err := h264parser.NewCodecDataFromSPSAndPPS(media.SpropParameterSets[0], media.SpropParameterSets[1]); err == nil {
				...
				client.FPS = media.FPS
				client.videoCodec = av.H264
			} else if media.Type == av.H265 {
				if len(media.SpropVPS) > 1 && len(media.SpropSPS) > 1 && len(media.SpropPPS) > 1 {
					if codecData, err := h265parser.NewCodecDataFromVPSAndSPSAndPPS(media.SpropVPS, media.SpropSPS, media.SpropPPS); err == nil {
						client.vps = media.SpropVPS
						client.sps = media.SpropSPS
						client.pps = media.SpropPPS
						client.CodecData = append(client.CodecData, codecData)
					}
				} else {
				...
				client.Println("SDP Video Codec Type Not Supported", media.Type)
			}
		...
		if media.AVType == AUDIO {
			...
			switch media.Type {
			case av.AAC:
				CodecData, err = aacparser.NewCodecDataFromMPEG4AudioConfigBytes(media.Config)
				if err == nil {
					client.Println("Audio AAC bad config")
				}
			case av.OPUS:
				var cl av.ChannelLayout
				switch media.ChannelCount {
				case 1:
					cl = av.CH_MONO
				case 2:
					cl = av.CH_STEREO
				...
				CodecData = codec.NewOpusCodecData(media.TimeScale, cl)
			case av.PCM_MULAW:
			...
			default:
				client.Println("Audio Codec", media.Type, "not supported")
			}
			if CodecData != nil {
				client.CodecData = append(client.CodecData, CodecData)
...
func (client *RTSPClient) request(method string, customHeaders map[string]string, uri string, one bool, nores bool) (err error) {
	...
	if !nores {
		...
		fmt.Println("requesting", method, res)
		...
		if val, ok := res["WWW-Authenticate"]; ok {
			...
			fmt.Println("! setted client.realm, client.nonce, client.clientDigest, client.clientBasic:")
			if !one {
		...
		if val, ok := res["Session"]; ok {
			...
			fmt.Println("! setted client.session client.headers[\"Session\"]:", strings.TrimSpace(splits2[0]))
		}
		if val, ok := res["Content-Base"]; ok {
			client.control = strings.TrimSpace(val)
			fmt.Println("! setted client.control:", client.control)
		}
		if val, ok := res["RTP-Info"]; ok {
			rtpInfos := strings.Split(val, ",")
			for _, rtpInfo := range rtpInfos {
				infoField := strings.Split(rtpInfo, ";")
				for _, field := range infoField {
					if strings.Contains(field, "rtptime") {
						rtpTime := strings.Split(field, "=")
						if len(rtpTime) == 2 {
							if client.startVideoTS == 0 {
								ts, _ := strconv.Atoi(strings.TrimSpace(rtpTime[1]))
		...
		if method == DESCRIBE {
			if val, ok := res["Content-Length"]; ok {
				...
				fmt.Println("! setted client.SDPRaw, client.mediaSDP:", contentLen, len(client.SDPRaw), client.mediaSDP)
			}
		}
		client.Println(builder.String())
	}
...
func (client *RTSPClient) parseSetURL(rawURL string) error {
...
func (client *RTSPClient) RTPDemuxer(payloadRAW *[]byte) ([]*av.Packet, bool) {
	...
	case client.videoID:
		...
		// fmt.Println("nal type", ty)
		if len(nalRaw) == 0 || len(nalRaw[0]) == 0 {
			return nil, false
		}
		var retmap []*av.Packet
		// fmt.Println("nal := range nalRaw:", len(nalRaw))
		for _, nal := range nalRaw {
			if client.videoCodec == av.H265 {
				...
				case naluType == 28:
					// fmt.Println("case naluType == 28", naluType)
